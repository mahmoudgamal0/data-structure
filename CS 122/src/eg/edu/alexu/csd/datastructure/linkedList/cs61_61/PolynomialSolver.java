...
public class PolynomialSolver implements IPolynomialSolver {
		...
		if (i < listA.size()) {
			for (int k = i; k < listA.size(); k++) {
				PolyType entry = new PolyType(((PolyType) listA.get(k)).getCoeff(),
						((PolyType) listA.get(k)).getExpo());
				this.R.add(entry);
			}
		} else {
			for (int k = j; k < listB.size(); k++) {
				PolyType entry = new PolyType(((PolyType) listB.get(k)).getCoeff(),
						((PolyType) listB.get(k)).getExpo());
				this.R.add(entry);
			}
		}
	...
	public int[][] subtract(char poly1, char poly2) {
		...
		if (i < listA.size()) {
			...
			for (int k = j; k < listB.size(); k++) {
				PolyType entry = new PolyType(-1 * ((PolyType) listB.get(k)).getCoeff(),
						((PolyType) listB.get(k)).getExpo());
				this.R.add(entry);
			}
		}
